...
class ReportGenerator {
     ...
     */
    static generatePDFReport(campaign) {
     ...
     */
    static addSummaryStats(doc, campaign) {
        // Get execution data
        ...
        const stats = [
            ...
            {
                label: 'Success Rate',
                value: `${((executionData.completed / Math.max(executionData.totalRequests, 1)) *
                    100).toFixed(1)}%`,
            },
        ];
        stats.forEach(stat => {
     ...
     */
    static addDeliveryTable(doc, campaign) {
     ...
     */
    static addErrorDetails(doc, campaign) {
        const executionData = this.getExecutionData(campaign);
    ...
    static addFooter(doc) {
        ...
        for (let i = 0; i < pages.count; i++) {
            ...
            doc
                .fontSize(8)
                .font('Helvetica')
                .text(`Page ${i + 1} of ${pages.count}`, 40, doc.page.height - 30, { align: 'center' });
            doc.text(`Generated: ${new Date().toLocaleString()}`, 40, doc.page.height - 20, { align: 'center' });
        }
    }
    /**
     * Build delivery matrix: contact × message → status
     */
    static buildDeliveryMatrix(campaign) {
        ...
        const allQueues = queue_manager_1.QueueManager.getAllQueues();
        for (const queueData of Object.values(allQueues)) {
            if (!queueData.requests)
                continue;
            for (const request of queueData.requests) {
                // Find matching delivery entry
                const chatId = request.request.data.chatId;
                ...
                for (const [key, delivery] of deliveryMap.entries()) {
                    if (delivery.contact === cleanContact && request.campaignId === campaign.campaignId) {
                        ...
                        for (let i = 0; i < campaign.totalMessages; i++) {
                            if (campaign.wahaRequests.findIndex(wr => wr.data.session === delivery.session &&
                                wr.data.chatId === chatId &&
                                campaign.wahaRequests.indexOf(wr) % campaign.totalMessages === i) >= 0) {
                                messageIndex = i;
                                break;
                            }
                        }
     ...
     */
    static getExecutionData(campaign) {
        let completed = 0;
        let failed = 0;
        let pending = 0;
        const allQueues = queue_manager_1.QueueManager.getAllQueues();
     ...
     */
    static getFailedRequests(campaign) {
        const failures = [];
        const allQueues = queue_manager_1.QueueManager.getAllQueues();
        for (const queueData of Object.values(allQueues)) {
            if (!queueData.requests)
